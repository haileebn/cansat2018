...
void getBackupData(){
  for(int i = 0; i < EEPROM.length(); i = i +32){
      EEPROM.get(i, data2);
      Serial.println(String("{\"time\":\"" + String(data2.timestamp) + "\",") 
                  + String("\"values\":{")
                  + String("\"LAT\":\"" + String(data2.lat,10) + "\",")
                  + String("\"LON\":\"" + String(data2.lon,10) + "\",")
                  + String("\"temp\":\"" + String(data2.temp) + "\",")
                  + String("\"hud\":\"" + String(data2.hud) + "\",")
                  + String("\"pressure\":\"" + String(data2.pressure) + "\",")
                  + String("\"pm1\":\"" + String(data2.pm1) + "\",")
                  + String("\"pm25\":\"" + String(data2.pm25) + "\",")
                  + String("\"pm10\":\"" + String(data2.pm10) + "\",")
                  + String("\"CO\":\"" + String(data2.co) + "\",")
                  + String("\"al\":\"" + String(data2.al) + "\",")
                  + String("\"dem\":\"" + String(data2.dem) + "\"")
                  + String("}}"));
  }  
}
...
//  coValue = (analogRead(sensor)/1024)*5*200;
  ...
  data.al = int(getAltitudePressure(getTemperaturePressure())*100);
  if(data.al > 5000) {
    flag = 1;  
  }

//  Serial.println(getTemperaturePressure());
...
*/
double getAltitudePressure(double t){
